 ...
 */
function hook_isodmeproxmox_clientedit(array $params)
{
    try {
 ...
 */
add_hook('ClientEdit', 1, 'hook_isodmeproxmox_clientedit');
    ...
    if (!is_null($menu->getChild('Services'))) {
        ...
        $menu->getChild('Services')
            ->addChild(
                'My VPSes',
                array(
                    'uri' => '/clientarea.php?action=services&module=isodmeproxmox',
                    'order' => 15,
                )
            );
    }
...
add_hook('ClientAreaSecondarySidebar', 1, function ($secondarySidebar)
{
    ...
    if (!is_null($secondarySidebar->getChild('My Services Actions'))) {
        ...
        $customPanel = $secondarySidebar->addChild('New Storage VPS Line!');

        // set panel attributes
        $customPanel->moveToFront()
            ->setIcon('fa-server')
            ->setBodyHtml(
                'Come rent a storage VPS! High disk capacity, perfect for backups.'
            )
            ->setFooterHtml(
